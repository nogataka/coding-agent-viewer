...
export const useEventSourceManager = ({
  ...
  const processStatusRef = useRef<Map<string, string>>(new Map());
  ...
  useEffect(() => {
    if (!enabled || !processes.length) {
      ...
      processStatusRef.current.clear();
      return;
    }
    ...
    eventSourcesRef.current.forEach((es, id) => {
      if (!currentIds.has(id)) {
        ...
        processStatusRef.current.delete(id);
      }
    });

    // Add new connections
    processes.forEach((process) => {
      const previousStatus = processStatusRef.current.get(process.id);
      processStatusRef.current.set(process.id, process.status);

      const existingSource = eventSourcesRef.current.get(process.id);
      const shouldRestart =
        existingSource &&
        previousStatus &&
