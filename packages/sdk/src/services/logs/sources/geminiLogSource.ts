...
export class GeminiLogSource extends BaseExecutorLogSource {
  private readonly GEMINI_TMP_DIR = path.join(os.homedir(), '.gemini', 'tmp');
  ...
  private async findSessionFile(dir: string, sessionId: string): Promise<string | null> {
    try {
    ...
    try {
      ...
      for (const message of messages) {
        ...
        if (typeof message.content === 'string') {
          ...
          if ('text' in (message.content as Record<string, unknown>)) {
            text = String((message.content as { text?: unknown }).text ?? '');
          }
        }
      ...
      console.warn(
        `[GeminiLogSource] Failed to extract first user message from ${filePath}`,
        error
      );
    }
