...
export class ExecutionService {
    static SESSION_ID_RESOLUTION_TIMEOUT_MS = 30000;
    registry = new ProfileRegistry();
    activeProcesses = new Map();
    async startNewChat(request) {
        const sessionId = this.composeSessionId(request.executorType, request.actualProjectId);
        const launchRequest = { ...request, kind: 'new', sessionId };
        return this.launch(launchRequest);
    }
    async sendFollowUp(request) {
        const launchRequest = { ...request, kind: 'follow-up' };
        return this.launch(launchRequest);
    }
    async launch(request) {
        ...
        const { args, stdinPayload, createSessionIdResolver, resolveSessionId } = profile.buildProcessParameters?.(command, request) ??
        ...
        let resolveActualSessionUuid = () => { };
        let actualSessionUuid = null;
        ...
        child.once('exit', (code, signal) => {
            ...
            process.stdout.write(`Execution ${currentSessionId} exited with code ${code ?? 'null'} signal ${signal ?? 'null'}\n`);
        });
        ...
        if (sessionIdResolver || resolveSessionId) {
            ...
            const timeoutPromise = new Promise((resolve) => {
                timeoutId = setTimeout(() => resolve(mintedSessionId), ExecutionService.SESSION_ID_RESOLUTION_TIMEOUT_MS);
            });
            ...
            if (sessionIdResolver) {
                resolutionCandidates.push(actualSessionUuidPromise.then((uuid) => uuid
                    ? this.composeSessionId(request.executorType, request.actualProjectId, uuid)
                    : mintedSessionId));
            }
            if (resolveSessionId) {
                resolutionCandidates.push(resolveSessionId({
                    request,
                    mintedSessionId,
    ...
    composeSessionId(executorType, actualProjectId, sessionUuid) {
        const suffix = sessionUuid ?? randomUUID();
        return `${executorType}:${actualProjectId}:${suffix}`;
    }
    composeEnvironment(command, request) {
        const baseEnv = {
            ...process.env,
            ...command.env,
    ...
    buildDefaultProcessParameters(command, request) {
    ...
    updateProcessSessionId(oldId, newId, child) {
        if (oldId === newId) {
            return;
        }
