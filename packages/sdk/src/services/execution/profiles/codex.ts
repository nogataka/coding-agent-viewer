...
const readSessionMetaLine = async (filePath: string): Promise<CodexSessionMeta | null> => {
...
const findLatestCodexSessionUuid = async (
  ...
  let candidate: { uuid: string; mtimeMs: number } | null = null;
...
const resolveCodexSessionIdFromFilesystem = async (
  ...
  return {
    handleChunk(chunk: string): string | null {
      ...
      while (newlineIndex >= 0) {
        ...
        if (line.length > 0) {
          try {
            const parsed = JSON.parse(line) as { type?: string; payload?: { id?: unknown } };
...
const buildCodexParameters = (
  ...
  return {
    ...
    createSessionIdResolver:
      request.kind === 'new' ? () => createCodexSessionResolver() : undefined,
    resolveSessionId: request.kind === 'new' ? resolveCodexSessionIdFromFilesystem : undefined
  };
};

export const codexProfile: ProfileDefinition = {
  label: 'codex',
  command: {
    binary: 'npx',
    args: [
