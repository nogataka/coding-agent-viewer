...
import { CommandExitStatusType, ToolResultValueType } from '../executors/types';
...
function decodeCursorBlob(data) {
    if (!data.length) {
        return { kind: 'noop' };
    }
...
function createAddPatch(index, entry) {
    return {
        op: 'add',
        path: `/entries/${index}`,
        value: {
...
function createReplacePatch(index, entry) {
    return {
        op: 'replace',
        path: `/entries/${index}`,
        value: {
...
function stripWorkspaceTags(text) {
    return text
        .replace(/<workspace_result[^>]*>/gi, '')
        .replace(/<\/workspace_result>/gi, '')
        .trim();
}
function buildToolEntry(toolName, args, workspacePath) {
    ...
    switch (lower) {
        ...
        case 'grep': {
            const query = args.pattern ?? args.regex ?? '';
            return {
                toolLabel: 'Grep',
                actionType: {
                    action: 'search',
...
function extractTextFromCursorPayload(payload) {
    if (!payload.content) {
        return '';
    }
...
function applyToolResult(state, item) {
    if (!item.toolName) {
        return null;
    }
    const baseMetadata = state.metadata && typeof state.metadata === 'object'
        ? state.metadata
        : {};
    const metadata = {
        ...baseMetadata,
...
function hasReadableCharacters(text) {
...
function stripTaggedContent(text, tag) {
...
export class CursorLogSource extends BaseExecutorLogSource {
    CURSOR_CHATS_DIR = path.join(os.homedir(), '.cursor', 'chats');
    CURSOR_PROJECTS_DIR = path.join(os.homedir(), '.cursor', 'projects');
    workspacePathCache = null;
    getName() {
        return 'CURSOR';
    }
    async loadWorkspacePathCache() {
    ...
    async getWorkspacePathFromHash(hash) {
        const cache = await this.loadWorkspacePathCache();
        return cache.get(hash) ?? null;
    }
    async resolveSessionFilePath(executionId, sessionId, workingDir) {
    ...
    async streamCompletedSession(filePath) {
        ...
        try {
                ...
                if (decoded.kind === 'json') {
                    ...
                    if (role === 'user') {
                        ...
                        if (Array.isArray(payload.content)) {
                            ...
                            combined = sanitizeText(payload.content);
                        }
            ...
            const workspacePath = (await this.getWorkspacePathFromHash(hashSegment)) ?? path.dirname(path.dirname(filePath));
            ...
            for (const event of assistantEvents) {
                ...
                for (const item of payload.content ?? []) {
                    ...
                    if (itemType === 'reasoning') {
                        ...
                        if (reasoningSeen.has(signatureKey))
                            continue;
                    ...
                    if (itemType === 'tool-call') {
                        ...
                        const { toolLabel, actionType, content } = buildToolEntry(item.toolName, args, workspacePath);
                        const entry = {
                            timestamp: null,
                            entry_type: {
                                type: 'tool_use',
            ...
            for (const event of toolEvents) {
                for (const item of event.payload.content ?? []) {
                    if (!item || !item.type || item.type.toLowerCase() !== 'tool-result')
                        continue;
                    if (!item.toolCallId)
                        continue;
                    const callId = sanitizeText(item.toolCallId);
     ...
     * 指定プロジェクトのセッション一覧を取得
     */
    extractFirstUserMessage(storeDbPath) {
        let db = null;
        try {
            ...
            logger.debug(`[CursorLogSource] Failed to extract first user message from ${storeDbPath}`, error);
        }
        finally {
            db?.close();
        }
...
//# sourceMappingURL=cursorLogSource.js.map