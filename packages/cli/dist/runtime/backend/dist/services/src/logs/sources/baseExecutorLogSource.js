 ...
 */
export class BaseExecutorLogSource {
    /**
    ...
    async streamLiveSession(filePath) {
        ...
        try {
            ...
            if (stats.size > 0) {
                ...
                logger.info(`[${this.getName()}] Initial read: ${lines.length} lines, ${lastPosition} bytes`);
            }
        }
        catch (err) {
     ...
     * エントリをJSON Patchに変換
     */
    convertEntryToJsonPatch(entry, entryIndex) {
        ...
        if (typ === 'response_item' && payload.type === 'function_call_output') {
            ...
            return {
                ...
                entry_type: this.createToolUseEntryType(toolLabel, {
                    action: 'tool',
                    tool_name: toolLabel,
                    result: parsedOutput && typeof parsedOutput === 'object'
    ...
    formatToolArgumentContent(args) {
    ...
    createToolUseEntryType(toolName, action) {
        return {
            type: 'tool_use',
            tool_name: toolName,
            action_type: action
    ...
    createCommandResult(output, exitStatus) {
        const result = {
            output
        };
    ...
    extractTextContent(content) {
        ...
        if (Array.isArray(content)) {
            const joined = content
                ...
                if (item && typeof item === 'object' && 'text' in item) {
                    return this.sanitizeInstructionText(String(item.text ?? ''));
                }
        ...
        if (content && typeof content === 'object' && 'text' in content) {
            return this.sanitizeInstructionText(String(content.text ?? ''));
        }
...
//# sourceMappingURL=baseExecutorLogSource.js.map