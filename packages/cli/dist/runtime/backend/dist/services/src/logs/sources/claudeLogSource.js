...
import { ToolResultValueType, CommandExitStatusType } from '../executors/types';
...
export class ClaudeLogSource extends BaseExecutorLogSource {
     ...
     */
    transformWorktreePath(workingDir) {
    ...
    async getSessionList(projectId) {
        try {
            ...
            for (const entry of entries) {
                    ...
                    for (const line of lines) {
                        try {
                            ...
                            if (!workspacePathFromLog) {
                                const candidate = (typeof parsed.cwd === 'string' &&
                ...
                const fallbackWorkspace = fallbackWorkspaceRaw && fallbackWorkspaceRaw.length > 0
                    ? fallbackWorkspaceRaw.startsWith('/')
                        ? fallbackWorkspaceRaw
                        : `/${fallbackWorkspaceRaw}`
                    : null;
                const workspacePath = workspacePathFromLog && workspacePathFromLog.length > 0
    ...
    async streamLiveSession(filePath) {
        this.resetState();
    ...
    transformClaudeLine(line, entryIndexRef) {
        ...
        if (json.type === 'assistant' && json.message) {
            ...
            for (const part of messageParts) {
                if (part.type === 'text' || part.type === 'analysis' || part.type === 'thinking') {
                    ...
                    const entry = {
                        timestamp: null,
                        entry_type: part.type === 'thinking' ? { type: 'thinking' } : { type: 'assistant_message' },
                        content: text,
                        metadata: part
                    };
                    operations.push({
    ...
    createToolEntryFromClaude(part, cwd, timestamp) {
        const toolName = String(part.name ?? part.tool ?? 'tool');
        const toolId = part.id ?? part.tool_use_id ?? null;
    ...
    mapToolToAction(toolName, input, cwd) {
        const lower = toolName.toLowerCase();
        const makePath = (candidate) => {
...
//# sourceMappingURL=claudeLogSource.js.map