...
export class FilesystemWatcher extends EventEmitter {
    ...
    async startWatching(watchPath) {
        ...
        try {
            ...
            throw new FilesystemWatcherError(`Failed to start watching ${resolvedPath}: ${error}`, 'START_WATCH_FAILED');
        }
    }
    /**
     * Stop watching
     ...
     */
    async buildGitignoreRules(rootPath) {
        try {
     ...
     */
    async processGitignoreFiles(rootPath) {
        try {
     ...
     */
    async findGitignoreFiles(rootPath, maxDepth = 10) {
        const gitignoreFiles = [];
        const searchDir = async (dirPath, currentDepth = 0) => {
     ...
     */
    handleDebouncedEvent(eventType, filePath, stats) {
     ...
     */
    async getAllWatchedFiles(dirPath) {
        const files = [];
...
let globalFilesystemWatcher = null;
export function getGlobalFilesystemWatcher() {
