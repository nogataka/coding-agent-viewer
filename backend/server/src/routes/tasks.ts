...
import type { SessionInfo } from '../../../services/src/logs/logSourceStrategy';
...
const SESSION_STATUS_TO_TASK_STATUS: Record<string, TaskStatus> = {
  running: 'inprogress',
  completed: 'done',
  failed: 'inreview'
};

const mapSessionStatusToTaskStatus = (status: SessionInfo['status']): TaskStatus =>
  SESSION_STATUS_TO_TASK_STATUS[status] ?? 'done';

const mapExecutorTypeToProfile = (executorType: string): string =>
  EXECUTOR_PROFILE_MAP[executorType] || executorType.toLowerCase();

const sessionInfoToTask = (session: SessionInfo, executorType: string, projectId: string) => {
...
const extractTaskFromTaskWithStatus = (task: ReturnType<typeof sessionInfoToTask>) => ({
  id: task.id,
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const task = extractTaskFromTaskWithStatus(
      sessionInfoToTask(result.session, result.executorType, result.projectCompositeId)
    );
