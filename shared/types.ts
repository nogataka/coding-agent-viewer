// Shared TypeScript contracts consumed by both the backend and frontend.
// Keep changes in sync with backend validators and API responses.
// Coordinate updates with frontend stores/components that consume these types.
...
export type UpdateTaskTemplate = {
  title: string | null;
  description: string | null;
...
export type CreateTask = {
  ...
  profile: ProfileVariantLabel | null;
};
...
export type Mycode = { command: CommandBuilder; append_prompt: string | null };
...
export type CreateTaskAttemptBody = {
  task_id: string;
  profile_variant_label: ProfileVariantLabel | null;
  base_branch: string;
};
