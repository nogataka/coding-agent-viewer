...
if (!fs.existsSync(backendEntry)) {
  console.error('❌ Backend build not found. Run "npm run build:npx" before publishing.');
  process.exit(1);
}

if (!fs.existsSync(frontendDir)) {
  console.error('❌ Frontend build not found. Run "npm run build:npx" before publishing.');
  process.exit(1);
}
